...
files <- read.csv("C:\\Users\\91628\\OneDrive\\Documents\\ProgrammingLanguages\\R\\r_practice\\data.csv")
files
head(files)
View(files)
...
#R uses paste() and paste0() functions to format strings and variables together
#for printing in few different ways

print(paste('hello','R'))
print(paste('hello','R' , sep=","))
paste0('hello','me')
#paste () — this function concatenates the input values and returns a single 
#character string. paste0 () — this function concatenates the input values in a 
#single character string. The difference between: paste and paste0 is that paste
#function provides a separator operator, whereas paste0 does not
...
#factor vector
fac = as.factor(ltrs)
fac #factor vector fac based on the unique values in ltrs
class(fac)
...
#This is implicit coercion where R tries to find a way to represent all obj in 
#the vector in a reasonable func
...
#to add 2 vectors
new_order <- c(5,10)
#When you try to add two vectors of different lengths, R will recycle the 
#shorter vector to match the length of the longer vector. This means new_order 
#will be repeated to match the length of order_detail.
#Here’s what happens:
#order_detail is c(10, 20, 30, NA, 50, 60)
#new_order is c(5, 10), which will be recycled to c(5, 10, 5, 10, 5, 10)
