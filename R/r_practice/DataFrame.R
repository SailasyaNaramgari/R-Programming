...
#Default Behavior: In older versions of R (prior to R 4.0.0), when you created a 
#data frame, any character vector was automatically converted into a factor 
#(a categorical variable) by default. This means that strings were treated as 
#a categorical variable with levels, which could lead to issues if you wanted to 
#keep them as character strings.

#stringsAsFactors = FALSE: By setting stringsAsFactors = FALSE, you instruct R 
#to keep character vectors as character vectors instead of converting them into
#factors. This is generally preferred when you want to maintain the integrity of
#the string data without converting it to a categorical variable.
...
df[2] #single bracket result in df
...
#The runif() function in R is used to generate random numbers following a uniform distribution. 
